...
Phase 2C adds `local_llm_provider.py` and wires an application-cached `LocalLLMProvider` into the existing mode-aware unresolved seam. The same configured provider is reused across runs, while the MLX runtime and model still load only when the first unresolved invoice needs them. Qwen3 thinking is disabled in the chat template, and generation is greedy and token-bounded. The adapter accepts exactly the existing invoice JSON keys, rejects wrappers/schema drift, and rejects factual model fields that cannot be grounded in the local source text. Deterministic values remain authoritative during merge, with field provenance and conflicts retained in extraction trace data.

Phase 2D adds `recognition_validation.py`. Deterministic fast paths, local Qwen output, and authorized cloud results now pass through the same typed gate before archive. It validates required fields by document type, real calendar dates, supported domestic and foreign invoice-number shapes, labelled taxpayer identifiers, OCR confidence, merge conflicts, and labelled subtotal/tax/total arithmetic using `Decimal` with a `0.01` tolerance. Negative red-invoice amounts remain valid; zero and non-string/non-Decimal amounts require review. Recognized exempt supporting documents such as ride itineraries remain compatible even when their legacy `is_invoice` flag is false. The Qwen adapter does not expose a calibrated field-confidence value, so the gate does not invent one; it instead relies on exact-schema parsing, source grounding, field conflicts, and the real OCR confidence when available. Internal accepted/review/failed states map to resolved/manual_review/unresolved, and only safe reason codes, field names, provenance, source kind, and confidence—not source text—enter routine trace data. DeepSeek adaptation, per-file re-recognition, and OFD ingestion remain pending.
...
4. **Implemented:** merge deterministic parser and local-model fields, validate with `Decimal` and conservative supported formats, and map results to accepted/review/failed. Invalid deterministic fields are excluded before a provider retry; conflicts and validation reasons remain traceable.
